...
@pytest.mark.parametrize("file_path, mock_json, expected", [
    ("valid_file.json", [{"operationAmount": {"currency": {"code": "RUB"}, "amount": 100}}],
...
@pytest.mark.parametrize("transactions, mock_currency_response, expected_sum", [
    ([{"operationAmount": {"currency": {"code": "RUB"}, "amount": 100}}],
     {"result": 100}, 100.00),  # Транзакция в RUB
    ([{"operationAmount": {"currency": {"code": "USD"}, "amount": 50}}],
     {"result": 4000}, 4000.00),  # 50 USD = 4000 RUB
    ([{"operationAmount": {"currency": {"code": "EUR"}, "amount": 80}}],
     {"result": 7000}, 7000.00),  # 80 EUR = 7000 RUB
])
